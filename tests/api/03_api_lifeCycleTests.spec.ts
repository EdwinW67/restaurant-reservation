...
test('can create and then delete a reservation (admin)', async ({
    request,
}) => {
    ...
    const all = (await listRes.json()) as Reservation[];
    const found = all.find((r) => r.id === id);
    expect(found).toBeFalsy();
});

test('can create, edit and then delete a reservation (admin)', async ({
    request,
}) => {
    // 1) Admin login to establish the session cookie in this request context
    const adminUser = process.env.ADMIN_USER ?? 'admin';
